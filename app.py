...
# --- 페이지 설정 ---
st.set_page_config(page_title="AI MV Director (Ultimate)", layout="wide")
...
with st.sidebar:
    ...
    # 2. [NEW] Gemini 모델 선택 (DeBrief 스타일)
    st.subheader("🧠 기획 모델 (Gemini)")
    st.caption("실패 시 하위 버전으로 자동 재시도합니다.")
    
    # DeBrief 앱에 있는 모델 + 안정적인 모델 리스트
    gemini_options = [
        "gemini-2.0-flash-lite-preview-02-05", # 최신 (빠름)
        "gemini-1.5-flash",        # 표준 (안정적)
        "gemini-1.5-pro",          # 고성능
        "gemini-1.5-flash-8b",     # 초경량
        "gemini-1.0-pro"           # 구버전 (최후의 보루)
    ]
    selected_gemini_model = st.selectbox("우선 시도할 모델", gemini_options, index=1)
...
# --- 메인 타이틀 ---
st.title("🎬 AI MV Director (Ultimate)")
st.subheader("강력한 모델 폴백(Fallback) & 고화질 스토리보드")

topic = st.text_area("영상 주제 입력", height=80, placeholder="예: 2050년 사이버펑크 서울, 비 오는 밤, 고독한 형사")

# ------------------------------------------------------------------
# [핵심] DeBrief 앱의 로직 이식 (모델 폴백 시스템)
# ------------------------------------------------------------------
...
def generate_with_fallback(prompt, api_key, start_model):
    ...
    # 2. 백업 모델 리스트 (DeBrief 앱 참조 + 사용자 요청 하위 버전 포함)
    # 상위 모델이 실패하면 아래 순서대로 시도합니다.
    backups = [
        "gemini-2.0-flash-lite-preview-02-05", 
        "gemini-1.5-flash", 
        "gemini-1.5-flash-latest", # 요청하신 latest 버전
        "gemini-1.5-flash-8b", 
        "gemini-1.0-pro",          # 구버전
        "gemini-flash-latest"      # 아주 오래된 별칭
    ]
    ...
    for model_name in fallback_chain:
        try:
            ...
            last_error = e
            # 실패 시 로그만 남기고(콘솔) 멈추지 않음 -> 다음 모델 시도
            print(f"⚠️ {model_name} 실패 -> 다음 모델 시도... (Error: {e})")
            time.sleep(0.5)
            continue
            
    # 리스트의 모든 모델이 실패했을 때만 에러 발생
...
# ------------------------------------------------------------------
# [유지] Hugging Face 이미지 생성 (기존 성공 버전)
# ------------------------------------------------------------------
