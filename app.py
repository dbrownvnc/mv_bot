...
# --- 메인 타이틀 ---
st.title("🎬 AI MV Director (Robust)")
st.subheader("DeBrief급 강력한 기획안 생성 & 고화질 스토리보드")

topic = st.text_area("영상 주제 입력", height=80, placeholder="예: 2050년 사이버펑크 서울, 비 오는 밤, 고독한 형사")

# ---------------------------------------------------------
# [핵심] DeBrief (app_final_v84.py)의 Gemini 로직 이식
# ---------------------------------------------------------
...
# DeBrief 앱에서 가져온 'generate_with_fallback' 함수 원본 적용
def generate_with_fallback(prompt, api_key, start_model):
    genai.configure(api_key=api_key)
    fallback_chain = [start_model]
    
    # DeBrief 앱의 백업 모델 리스트 그대로 사용
...
def generate_plan_gemini(topic, api_key):
    try:
        ...
        """
        # DeBrief 앱 방식대로 호출 (시작 모델 지정)
        response_text, used_model = generate_with_fallback(prompt, api_key, "gemini-2.0-flash")
        
        # 디버깅용: 사용된 모델 확인 (필요시 주석 해제)
        # st.toast(f"Used Model: {used_model}")
...
# ---------------------------------------------------------
# [이미지] Hugging Face 생성 (Pollinations 제한 회피)
# ---------------------------------------------------------
